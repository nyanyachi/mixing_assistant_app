...
def generate_txt_report(file_name, result, T):
    separator = "-" * 48

    lines = [
        "Mixing Assistant Report",
        "Generated by Mixing Assistant v0.4",
        separator,
        "",
        "[File]",
        f"Uploaded file: {file_name}",
        f"{T['duration']}: {result['duration_sec']:.2f} sec",
        f"{T['sample_rate']}: {result['sample_rate']} Hz",
        "",
        "[Basic Analysis]",
        f"{T['peak']}: {result['peak_db']:.2f} dB",
        f"{T['rms']}: {result['rms_db']:.2f} dB",
        f"{T['volume_variation']}: {result['volume_variation_db']:.2f} dB",
        separator,
        "",
        "[Compressor]",
        f"{T['compressor_need']}: {result['compressor_score']}/100 - {get_need_label(result['compressor_score'])}",
        f"Ratio: {result['compressor_ratio']}",
        f"Threshold: {result['compressor_threshold']}",
        f"Attack: {result['compressor_attack']}",
        f"Release: {result['compressor_release']}",
        f"Makeup Gain: {result['compressor_makeup_gain']}",
        separator,
        "",
        "[De-Esser]",
        f"{T['deesser_need']}: {result['deesser_score']}/100 - {get_need_label(result['deesser_score'])}",
        f"{T['sibilance_frequency']}: {result['sibilance_percent']:.1f}%",
        T["deesser_note"],
        separator,
        "",
        f"[{T['eq_assistant']}]",
        *result["eq_suggestions"],
        ...
        "",
        f"[{T['mix_ready']}]",
        f"{result['mix_score']}/100 - {get_mix_label(result['mix_score'])}",
        separator,
        "",
        f"[{T['quick_start']}]",
        *result["quick_start"],
...
def get_txt_report_filename(file_name):
    base_name = Path(file_name).stem
    base_name = base_name.replace(" ", "_")

    return f"{base_name}_mixing_report.txt"
...
if uploaded_file is not None:
    ...
    txt_report = generate_txt_report(uploaded_file.name, result, T)
    txt_file_name = get_txt_report_filename(uploaded_file.name)

    st.download_button(
        label=T["download_txt"],
        data=txt_report.encode("utf-8-sig"),
    ...
    st.info(T["waiting"])
    st.subheader(T["pre_upload_title"])
    st.markdown(T["pre_upload_guidance"])
